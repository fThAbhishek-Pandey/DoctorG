...
const MyApointment = () => {
  ...
  const getUserAppointments = async ()=>{
    try {
       const {data} = await axios.get(backendURL+ '/api/user/appointments', {headers:{user_token}})
      ...
      <div>
        ...
        appointments &&  appointments.map((item ,index)=>(
            <div className='grid grid-cols-[1fr_3fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                <div>
                  <img className='w-32 bg-indigo-50' src={item.docData.image} alt="docter image" />
                </div>
                <div className='flex-1 text-sm  text-zinc-600'>
                  ...
                  <p className='text-xs mt-1'><span className='text-sm text-neutral-700 font-medium'>Date And time</span> {slotDataFormate(item.slotDate)} | {item.slotTime}</p>
                </div>
                <div></div>
